...
///         reports from a set of INDEPENDENT updater keys. Manipulation-resistant
///         by design — a minority of compromised/faulty updaters cannot move the
///         median — which is the production replacement for the single-key
///         `ShadowPriceOracle` (whose single `UPDATER_ROLE` setter was flagged as a
///         manipulation vector, the JELLY-incident class of risk).
///
///         Guarantees consumed by SpotVault's fail-closed `_oraclePrice`:
///           - every report is bounded to [minAnswer, maxAnswer] (sanity bounds);
///           - `latestRoundData` REVERTS unless at least `minQuorum` reports are
///             fresh (within `maxStaleness`) — so a stalled feed fails closed;
///           - it returns the OLDEST contributing timestamp, so a downstream
///             staleness guard stays conservative.
///
/// @dev    Alternative: point `SpotVault.oracle` directly at a third-party push feed
///         (Chainlink / Stork / RedStone on HyperEVM — all listed in Hyperliquid's
///         builder-tools docs) if/when a live feed address is confirmed. This
///         contract is the self-operated option that removes that dependency.
///         Decimals follow the USD-feed convention (8). Median is computed over a
///         small updater set (insertion sort), cheap enough for on-chain NAV reads.
contract MedianOracle is AccessControl, AggregatorV3Interface {
    ...
    // ─── Updater: push an observation ─────────────────────────────────────

    /// @notice An updater submits its latest observed price. Bounded; the median
    ///         of all fresh observations is what `latestRoundData` returns.
    ...
    /// @dev Median of the first `count` elements (insertion sort; small set).
