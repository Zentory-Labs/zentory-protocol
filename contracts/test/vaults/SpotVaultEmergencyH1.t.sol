...
contract SpotVaultEmergencyH1Test is Test {
    ...
    function _accrueFee() internal {
        // Rebalance to fully long, then push mark up to create alpha above HWM.
        vault.rebalanceTo(10_000);
        oracle.setPrice(60_000 * 1e8); // 20% gain
        vault.evaluateFees();
    }
    ...
    function test_emergencyRedeem_totalAssetsStaysNonNegative() public {
        ...
        // totalAssets is settle-able: idle - accrued, clamped at zero.
        uint256 ta = vault.totalAssets();
        uint256 idle = wbtc.balanceOf(address(vault));
        uint256 accrued = vault.performanceFeeAccrued();
        assertEq(ta, idle > accrued ? idle - accrued : 0, "totalAssets invariant holds");
    }
}
