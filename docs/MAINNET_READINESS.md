...
| Stale-price window lets a depositor/redeemer extract mispricing from everyone else. | Value transfer between users; classic oracle-latency arbitrage. | E | 🟢 closed (Q9 fix — branch `fix/0-a-9-stale-price-twap`; `SpotVault` computes a time-weighted average price (TWAP) over a rolling 30-min window of recent on-chain observations and rejects any new price whose deviation from the TWAP exceeds `maxOracleDeviationBps` (default 1000 = 10%; admin-settable via `setMaxOracleDeviationBps`). The MedianOracle freshness bound (`maxOracleStaleness`) is the FIRST line of defence; the deviation guard is the SECOND. The guard is the Kept boundary check that closes the stale-price-window arbitrage from `AUDIT_FINDINGS_2026-08-07.md` (finding #4). 15 new tests in `contracts/test/vaults/TwapCheck.t.sol`; 2 new fuzz tests in `contracts/test/fuzz/SpotVaultNavMonotonicity.fuzz.t.sol`. Full suite passes 411/0/1.) |
| No per-depositor high-water-mark equalization. | Late depositors are retroactively taxed on gains they never received. | E | 🔴 |
| **`zBTC/zETH/zSOL/zXRP` cannot execute the strategy at all — yet charge a 20% performance fee and advertise "3x".** | Charging a performance fee for a strategy that cannot run is the single most indefensible thing here in diligence *and* to a regulator. | F+E | 🟡 partial — SpotVault v2 (`7ee6f92`) and the on-chain rebalance path landed; legacy z-vaults still need deprecation or live execution. |
| Oracle going quiet freezes **all** withdrawals; no fallback, no admin override. | Users cannot exit. "Can't withdraw" ends a protocol. | E | 🟢 closed (Q8 fix — branch `fix/0-a-8-admin-emergency-exit`; `SpotVault.redeemEmergencyFor(address owner, uint256 shares, address receiver)` admin variant callable by `RISK_COUNCIL_ROLE` for incident response. Preserves per-address MEV cooldown (admin path uses the same `lastEmergencyRedeemAt[owner]` key as the user path — admin cannot bypass the cooldown for a victim). Circuit-breaker halts admin path explicitly; admin respects ERC-4626 allowance (cannot drain a victim who has revoked). 11 new tests in `contracts/test/vaults/SpotVaultEmergencyAdminOverride.t.sol`; full suite passes 394/0/1.) |
