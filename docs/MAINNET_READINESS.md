...
| `accuracyCache` defaults to `0`, and **0 is the maximum-slash input** — an unscored signal is indistinguishable from a maximally-wrong one. | A dead scoring oracle silently burns every provider's stake. | E | 🟢 closed (Q6 fix — branch `fix/0-a-6-accuracy-cache-default`; sentinel `accuracyScored[signalId]` makes `applyPayout` revert with `SignalNotScored` for never-scored signals, plus a `claimExpiredSignal(signalId)` recovery path gated by `EPOCH_SETTLER` with `MAX_SIGNAL_AGE = 7 days` so keepers can clear permanently-stale entries without affecting stake. 12 new tests in `contracts/test/signals/EpochScoringExpiredSignal.t.sol`; 7 pre-existing tests in `EpochScoringPayoutReplay.t.sol` continue to pass.) |
